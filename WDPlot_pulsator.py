...
def main(generate):
    ...
    for idx_plot in range(len(pulsatornames)):
        ...
        if other_band_exists:
            ...
            flux_bgsub_other = alldata_flux_bgsub_other[idx_exposuregroup_other]
            flux_bgsub_err_other = alldata_flux_bgsub_err_other[idx_exposuregroup_other]
        ...
        ax.errorbar(jd_t_mean, flux_bgsub, yerr=flux_bgsub_err, color=bandcolors[band], marker='.', ls='', zorder=4, label=band)
        ...
        if len(redpoints) != 0: 
            ...
            ax.errorbar(jd_t_mean_red, flux_bgsub_red, yerr=flux_bgsub_err_red, color='#808080', marker='.', ls='', zorder=2, alpha=.5, label='Flagged')
        if len(bluepoints) != 0: 
        ...
        if other_band_exists:
            if len(flux_bgsub_other) != 0:
                ...
                ax.errorbar(jd_t_mean_other, flux_bgsub_other+2*max(flux_bgsub), yerr=flux_bgsub_err_other, color=bandcolors[band_other], marker='.', ls='', zorder=1, label=band_other, alpha=.25)
                ax.axhline(y=2*max(flux_bgsub), alpha=.15, ls='dotted', color=bandcolors[band_other])
        ...
        if other_band_exists and (len(flux_bgsub_other) != 0):
            ax.set_ylim(ymax = max(flux_bgsub_other+ 2*max(flux_bgsub))*1.2)
        else:
            ax.set_ylim(ymax = max(flux_bgsub)*1.2)

        ax.annotate(str(labelnum), xy=(.95, .85), xycoords='axes fraction', color='xkcd:red', fontsize=20, horizontalalignment='center', verticalalignment='center', **afont, **txtkwargsw)
