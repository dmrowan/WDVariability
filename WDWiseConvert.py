...
def main(name, W1=None, W1err=None, W2=None, W2err=None, W3=None, W3err=None, W4=None, W4err=None):

    df = pd.DataFrame({'MainID':[name], 
                       'F1':[None], 'F1err':[None], 
                       'F2':[None], 'F2err':[None], 
                       'F3':[None], 'F3err':[None], 
                       'F4':[None], 'F4err':[None], 
    ...
    if W1 is not None:
        ...
        if W1err is not None:
            ...
            df.loc[0, 'F1err'] = abs(flux1err - flux1)
    ...
    if W2 is not None:
        ...
        if W2err is not None:
            ...
            df.loc[0, 'F2err'] = abs(flux2err-flux2)
    ...
    if W3 is not None:
        ...
        if W3err is not None:
            ...
            df.loc[0, 'F3err'] = abs(flux3err-flux3)
    ...
    if W4 is not None:
        ...
        if W4err is not None:
            ...
            df.loc[0, 'F4err'] = abs(flux4err-flux4)
            errors.append(abs(flux4err-flux4))

    if os.path.isfile('IRSA_wise.csv'):
        df_all = pd.read_csv("IRSA_wise.csv")
        df_all = df_all.append(df, ignore_index=True)
        df_all.to_csv("IRSA_wise.csv", index=False)
    else:
        df_all = df
        df_all.to_csv("IRSA_wise.csv", index=False)
    print(df_all)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--name", help="Source name", required=True)
    ...
    main(name=args.name, W1=args.W1, W1err=args.W1err, W2=args.W2, W2err=args.W2err, W3=args.W3, W3err=args.W3err, W4=args.W4, W4err=args.W4err)
