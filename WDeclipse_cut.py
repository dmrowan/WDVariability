...
desc="""
WDeclipse_cut.py: Search for pulsations in sources with eclipse. 
Use window to choose eclipse start and end points.
"""
#Main plotting function
def main(csvname, mode):
    assert(mode in ['duration', 'Duration', 
                    'ingress', 'Ingress', 
                    'egress', 'Egress'])
    ###Path assertions###
    ...
    try:
        best_df = pd.read_csv("/home/dmrowan/WhiteDwarfs/"
                               "InterestingSources/eclipse_group.csv")
    except:
        print("Run plot eclipse with generate option first")
        return
    
    if os.path.isfile("/home/dmrowan/WhiteDwarfs/"
                      +"InterestingSources/eclipse_timing.csv"):
        df_timing = pd.read_csv("/home/dmrowan/WhiteDwarfs/"
                                +"InterestingSources/eclipse_timing.csv")
    else:
        emptylength = len(best_df['MainID'])
        df_timing = pd.DataFrame({'MainID':best_df['MainID'], 
                                  'Duration':np.zeros(emptylength),
                                  'Ingress': np.zeros(emptylength),
                                  'Egress':np.zeros(emptylength),
                                })
    ...
    idx_best = np.where(best_df["MainID"] == source)[0]
    assert(len(idx_best) == 1)
    idx_best = idx_best[0]
    bestvisit = best_df['df_number'][idx_best]
    bestband = best_df['band'][idx_best]
    assert(band == bestband)
    ...
    plt.errorbar(jd_t_mean, cps_bgsub, yerr=cps_bgsub_err, 
    ...
    if len(bluepoints) != 0: #these points aren't blue either...
        ...
        plt.errorbar(jd_t_mean_blue, cps_bgsub_blue, yerr=cps_bgsub_err_blue, 
    ...
    #introduce offset here
        ...
        plt.errorbar(jd_t_mean_other, cps_bgsub_other+2*max(cps_bgsub), 
                     yerr=cps_bgsub_err_other, 
                     color=bandcolors[band_other], marker='.', ls='', 
                     zorder=1, label=band_other, alpha=.25)
    ...
    idx_jd_start = np.where(
            np.array(abs(jd_t_mean - point_jd_start)) == np.min(abs(jd_t_mean - point_jd_start)))[0][0]
    plt.axvline(x=jd_t_mean[idx_jd_start])
    point_jd_end = eclipsepoints[1][0]
    idx_jd_end = np.where(
            np.array(abs(jd_t_mean - point_jd_end)) == np.min(abs(jd_t_mean - point_jd_end)))[0][0]
    plt.axvline(x=jd_t_mean[idx_jd_end])
    plt.ylim(ymax=2)

    timedelta = jd_t_mean[idx_jd_end] - jd_t_mean[idx_jd_start]
    ...
    timedelta = timedelta * 1440
    verify = input("Use timedelta obtained in df [y/n] ")
    if verify in ['Y', 'y']:
        if mode in ['duration', 'Duration']:
            print("Adding duration time ", timedelta)
            df_timing.loc[idx_best, 'Duration'] = timedelta
        elif mode in ['ingress', 'Ingress']:
            print("Adding ingress time ", timedelta)
            df_timing.loc[idx_best, 'Ingress'] = timedelta
        else:
            print("Adding egress time ", timedelta)
            df_timing.loc[idx_best, 'Egress'] = timedelta

        df_timing.to_csv("/home/dmrowan/WhiteDwarfs/InterestingSources/eclipse_timing.csv")
    else:
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mode", 
                        help="select to find duration, ingress, or egress", 
